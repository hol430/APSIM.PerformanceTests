...
namespace APSIM.PerformanceTests.Portal
{
    public partial class Values : System.Web.UI.Page
    {
        ...
        private string GetPredictedObservedTests(int currentPoID, string variable)
        {
            ...
            foreach (vPredictedObservedTestsFormatted item in tests)
            {
                try
                {
                    ...
                    //{
                    //    itemCurrentStr = htmlCodeGreen;
                    //}
                    //else if (item.PassedTest == false)
                    //{
                    //    itemCurrentStr = htmlCodeRed;
                    //}
                    //else
                    //{
                    //    itemCurrentStr = htmlCodeBlack;
                    //}
                    //itemCurrentStr = itemCurrentStr + item.Current.ToString() + htmlCodeEnd;
            ...
            double[] acceptedXValues, double[] acceptedYValues, double maxXYValue, string annotationText)
        {
            ...
            acceptedSeries.Url = string.Format("Details.aspx?PO_Id={0}", currPO_ID);
            ...
            currentSeries.Url = string.Format("Details.aspx?PO_Id={0}", currPO_ID);
            ...
            annotation.X = 78;
            annotation.Y = 26;
            annotation.Url = string.Format("Details.aspx?PO_Id={0}", currPO_ID);
