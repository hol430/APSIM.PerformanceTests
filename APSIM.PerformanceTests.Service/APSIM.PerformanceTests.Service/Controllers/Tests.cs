...
namespace APSIM.PerformanceTests.Service.Controllers
{
    public class Tests
    {
        ...
        /// Run tests
        /// </summary>
        public static DataTable DoValidationTest(string PO_Name,  DataTable POtable)
        {
            ...
            if (AcceptedStats == null || POName != PO_Name)
            {
                POName = PO_Name;
                AcceptedStats = stats;
            ...
            for (int i = 0; i < AcceptedStats.Count(); i++)
                for (int j = 1; j < statNames.Count; j++) //start at 1; we don't want Name field.
                {
                    ...
                    AcceptedTable.Rows.Add(PO_Name,
