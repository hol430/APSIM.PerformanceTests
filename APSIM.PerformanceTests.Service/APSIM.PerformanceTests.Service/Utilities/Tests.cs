            ...
            try
            {
                ...
                if (acceptedStats.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow rowAccepted in acceptedStats.Rows)
                    {
                        ...
                        {
                            //modLMC 31-Jan-2018 - need to ensure that the 'Accepted' it is not null (this could mean that this Variable was dropped/renamed
                            // in the prior pull request, and as such needs to be excluded here)
                            if (rowAccepted["Accepted"] != DBNull.Value)
                            {
